...
import { composeSegments, levelFromPoints, plainAnswer, points, quality } from './logic';
...
function App(){
  ...
  return <div className="app-shell">
    <header className="topbar">
      ...
      <div className="brand-copy"><p className="eyebrow">Sentence forge</p><h1>Argument Builder</h1><p>Build a stronger argument one piece at a time.</p></div>
    ...
    <main>
      <section className="topic-strip" aria-labelledby="topic-heading"><div><span className="step">01</span><div><p className="kicker" id="topic-heading">Choose a discussion</p><h2>{topic.question}</h2></div></div><label><span>Topic</span><select value={topicId} onChange={e=>switchTopic(e.target.value)}>{topics.map((t,i)=><option value={t.id} key={t.id}>{String(i+1).padStart(2,'0')} · {t.title}</option>)}</select></label></section>
      ...
      <div className="layout">
        <div className="primary-column">
          <section className={`workspace ${dragging?'is-dragging':''}`} onDragOver={e=>e.preventDefault()} onDrop={drop} aria-labelledby="answer-heading">
            <div className="section-head"><div><p className="kicker"><span className="status-dot"/>Live build</p><h2 id="answer-heading">Current answer</h2></div><div className="workspace-toggles"><Toggle label="Sentence anatomy" checked={anatomy} onChange={setAnatomy}/><Toggle label="Teacher mode" checked={teacher} onChange={setTeacher}/></div></div>
            ...
            <p className="core-note">Core idea: <strong>{topic.basicSubject.charAt(0).toUpperCase()+topic.basicSubject.slice(1)} {topic.predicate}.</strong></p>
            {previous&&<div className="change-panel"><div><span className="change-icon">↗</span><div><strong>{changed?`${metadata[changed].name} ${active.includes(changed)?'added':'removed'}`:'Previous version restored'}</strong><p>{changed&&active.includes(changed)?metadata[changed].feedback:'The sentence structure has been updated.'}</p></div></div><details><summary>See previous version</summary><p>{previous}</p></details></div>}
          </section>
          ...
          {compare&&<section className="comparison" aria-labelledby="compare-title"><div className="section-head"><div><p className="kicker">Before & after</p><h2 id="compare-title">Basic vs expanded</h2></div><button className="icon-button" onClick={()=>setCompare(false)} aria-label="Close comparison">×</button></div><div className="comparison-grid"><article><span>Basic · A1</span><p>{plainAnswer(topic,[])}</p></article><article className="expanded"><span>Expanded · {level}</span><p>{segments.map((s,i)=><mark key={i} className={s.id==='claim'?'plain':''}>{s.breakBefore&&<><br/><br/></>}{s.text}</mark>)}</p></article></div></section>}

          {teacher&&<TeacherPanel active={active} selected={changed||active.at(-1)||null}/>}
        </div>

        <aside className="side-column">
          <section className="level-card"><div className="level-card-top"><div><p className="kicker">Complexity range</p><h2>{level} argument</h2></div><strong>{score}<small> pts</small></strong></div><div className="level-track"><span style={{width:`${Math.min(100,score/19*100)}%`}}/></div><div className="level-labels"><span>A1</span><span>A2</span><span>B1</span><span>B2</span><span>C1</span></div><p>{featureNames.length?`${level} features: ${featureNames.join(', ')}.`:'Add modules to expand the range of communicative functions.'}</p><small>Estimated speaking complexity for practice purposes.</small></section>
    ...
    <footer><div><strong>Argument Builder</strong><span>Start clear. Add one useful function. Speak with structure.</span></div><p>Built for focused English speaking practice.</p></footer>
...
function ModuleCard({id,topic,active,onToggle,onDrag}:{id:ModuleId;topic:(typeof topics)[number];active:boolean;onToggle:(id:ModuleId)=>void;onDrag:(id:ModuleId|null)=>void}){const m=metadata[id];const fragment=id==='precision'?topic.preciseSubject:id==='contrast'?topic.concession:topic[id];return <button className={`module-card ${active?'active':''}`} onClick={()=>onToggle(id)} draggable onDragStart={e=>{e.dataTransfer.effectAllowed='copy';onDrag(id)}} onDragEnd={()=>onDrag(null)} aria-pressed={active}><span className="module-top"><b>{m.name}</b><em>{active?'✓ Active':m.level}</em></span><q>{fragment}</q><small>{m.explanation}</small><span className="module-action">{active?'Remove module':'＋ Add module'}</span></button>}
...
function TeacherPanel({active,selected}:{active:ModuleId[];selected:ModuleId|null}){const id=selected&&active.includes(selected)?selected:active.at(-1);if(!id)return <section className="teacher-panel"><p className="kicker">Teacher mode</p><h2>Add a module to reveal teaching notes.</h2></section>;const m=metadata[id];return <section className="teacher-panel"><div className="section-head"><div><p className="kicker">Teacher mode · {m.level}</p><h2>{m.name}</h2></div><span className="teacher-badge">Teaching notes</span></div><div className="teacher-grid"><div><span>Function</span><p>{m.feedback}</p></div><div><span>Grammar structure</span><p>{m.grammar}</p></div><div><span>Follow-up question</span><p>{m.followUp}</p></div><div><span>Correction task</span><p>{m.correction}</p></div></div></section>}
