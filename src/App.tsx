...
function App(){
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      <div className="layout">
        <div className="primary-column">
          <section className={`workspace ${dragging?'is-dragging':''}`} onDragOver={e=>e.preventDefault()} onDrop={drop} aria-labelledby="answer-heading">
            <div className="section-head"><div><p className="kicker"><span className="status-dot"/>Live build</p><h2 id="answer-heading">Current answer</h2></div><div className="workspace-toggles"><Toggle label="Traduzione italiana" checked={showItalian} onChange={setShowItalian}/></div></div>
          ...
          <section className="controls" aria-label="Answer controls">
            <div className="essential-controls"><button onClick={undo} disabled={!history.length} title="Undo last change">↶ <span>Undo</span></button><button onClick={reset} disabled={!active.length}>↺ <span>Reset</span></button><button onClick={copy}>□ <span>Copy answer</span></button><button onClick={speak}>▶ <span>Read aloud</span></button></div>
            <details className="advanced-tools"><summary>Altri strumenti <span>＋</span></summary><div><button onClick={randomTopic}>⌁ Random topic</button><button onClick={()=>setCompare(v=>!v)} className={compare?'selected':''}>◫ Compare</button><button onClick={startChallenge} className={challenge?'selected':''}>◎ {challenge?'Next challenge':'Start challenge'}</button><Toggle label="Sentence anatomy" checked={anatomy} onChange={setAnatomy}/><Toggle label="Teacher mode" checked={teacher} onChange={setTeacher}/></div></details>
          </section>
        ...
        <aside className="side-column">
          <section className="level-card"><div className="level-card-top"><div><p className="kicker">Complexity range</p><h2>{level} argument</h2></div><strong>{score}<small> pts</small></strong></div><div className="level-track"><span style={{width:`${Math.min(100,score/28*100)}%`}}/></div><div className="level-labels"><span>A1</span><span>A2</span><span>B1</span><span>B2</span><span>C1</span></div><p>{featureNames.length?`${level} features: ${featureNames.join(', ')}.`:'La frase base parte da A2; i tasselli usano soprattutto strutture B1 e B2.'}</p><small>Stima orientativa basata su lessico, grammatica e funzioni comunicative; non è una certificazione CEFR.</small></section>
          <details className="quality-details"><summary><span>Argument quality</span><b>{Object.values(meters).reduce((a,b)=>a+b,0)}/20</b></summary><QualityMeters values={meters}/></details>
      ...
      <section className={`sticky-answer ${dragging?'is-dragging':''}`} onDragOver={e=>e.preventDefault()} onDrop={drop} aria-label="Current answer while choosing modules"><div className="sticky-answer-head"><span><i/>Frase in costruzione</span><strong>{level}</strong></div><p key={answer}>{answer}</p>{showItalian&&<small>{italianAnswer(topic,active)}</small>}<div className="sticky-meta"><span>{active.length} {active.length===1?'tassello':'tasselli'} attivi</span><button onClick={undo} disabled={!history.length}>↶ Annulla</button></div></section>

      <section className="tray" aria-labelledby="tray-title"><div className="tray-title"><div><span className="step">02</span><div><p className="kicker">Language modules</p><h2 id="tray-title">Choose the next piece</h2></div></div><p>La frase rimane visibile qui sopra. Tocca un tassello per aggiungerlo o rimuoverlo.</p></div>
